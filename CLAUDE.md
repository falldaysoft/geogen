...
- **export.py**: `export_scene(root, path, player=None)` → GLB/glTF (node hierarchy with local transforms, PBR textures, texture-space UVs) or OBJ+MTL+PNG. glTF exports also write `<name>.manifest.json` (format `geogen-manifest` v1: model file, units, up axis, player spec, `rooms`, `spawns`) — the contract the Godot runtime reads.
